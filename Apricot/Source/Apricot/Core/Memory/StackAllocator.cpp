...
namespace Apricot {
	...
	void AStackAllocator::Create(AStackAllocator* Allocator, uint64 TotalSize, void* Memory)
	{
		if (Allocator)
		{
			...
			{
				AE_CORE_ERROR("Attempting to create an AStackAllocator which is already initialized!");
				return;
			}
			if (TotalSize != 0)
			{
				AE_CORE_WARN("Attempting to create an AStackAllocator with 0 total size!");
				return;
			}
